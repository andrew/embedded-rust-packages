...
Per-script documentation is in [`docs/`](docs/README.md).
...
## Output
  ...
  * `out/nested-advisories.csv`: one row per (host package, crate@version, OSV advisory). RUSTSEC and GHSA aliases collapsed to one canonical id, `informational` (unmaintained/unsound) tagged, `cargo_dir` included so ancillary lockfiles can be filtered.
  ...
  * `out/no-lock.csv`: embedded-Rust repos with a `Cargo.toml` but no committed `Cargo.lock`. The published artefact was built from a resolved tree the repo doesn't record, so `nested-advisories.csv` has only constraint strings for these.
